...
At a given sample size $n$, our algorithm proceeds as follows: we run a test function in the $\textbf{A}$ matrix and in each ${\textbf{A}^i_{\textbf{B}}}$ matrix, thus obtaining $1+k$ vectors of size $n$ with the model output $Y$. The total number of runs to compute $S_{Ti}$ is thus $n(1 + k)$.
...
Let's recapitulate at this point to see how our algorithm allows to save some model runs. For instance, let's assume that $n=16$. At this stage, we have used a total of 88 model runs: $\frac{1}{4} n$ have been applied to the $\textbf{A}$ matrix and to each ${\textbf{A}^i_{\textbf{B}}}$ ($4 * 7=28$);  plus $\frac{3}{4} n$ have been applied to the $\textbf{A}$ matrix and the ${\textbf{A}^i_{\textbf{B}}}$ matrices of the 4 most important parameters ($12*5=60$). This means that we have 24 extra runs ($\big[n(k+1)\big] -88=24$; $112-88=24$) to divide into the 4 most important parameters to improve their estimation (6 extra model runs per parameter).

We then retrieve the ${\textbf{A}^i_{\textbf{B}}}$ matrices of those parameters that have not been fixed, and for each one of them, we select the 6 rows where we will compute the extra model runs. The $i-th$ column of each ${\textbf{A}^i_{\textbf{B}}}$ matrix is substituted by a vector of size $\frac{1}{4} n$ formed by randomly distributed numbers within [0, 1]. We will call this new matrices ${\textbf{A}^i_{\textbf{X}}}$ matrices. We compute the test function in each ${\textbf{A}^i_{\textbf{X}}}$ matrix, and obtain 4 vectors of size $\frac{1}{4} n$ with the model output $Y$ obtained using the new sampled points; this is the "extra sample" in our paper. 

At this stage, and for each non-fixed parameter, we have computed $n$ effects $f({\textbf{A}} - {\textbf{A}^i_{\textbf{B}}})$; a total of 64 if $n=16$.  With the new effects $f({\textbf{A}} - {\textbf{A}^i_{\textbf{X}}})$ and $f({\textbf{A}^i_{\textbf{B}}} - {\textbf{A}^i_{\textbf{X}}})$, we can add $\frac{1}{4}n$ * 2 (12) extra effects to each non-fixed model input, thus gaining 48 extra effects overall. Thus, for a sample size of $n=16$ (total sample size of 112), the $\frac{3}{4}$ most important factors have received 22 runs each; six extra runs each. The two less important factors have received $\frac{1}{4} n$ runs each (4 runs each). Jointly with the runs conducted on the $\textbf{A}$ matrix ($n$), this adds up to $n(k+1)$ runs, a total of 112. As for the number of effects, the less important factors have received a number of effects of $\frac{1}{4}n$ (4) each, and the most important factors have received $n + \frac{3}{4}n$ (28) each.

After computing the $S_{Ti}$ for the non-fixed parameters, we merge the results with the $S_{Ti}$ of the fixed parameters.
